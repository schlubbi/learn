...
void Database_find_by_name( Connection *conn, const char *name)
{
	Database *db = conn->db;
	for(int i = 0; i < MAX_ROWS; i++) {
		Address *current_address = &db->rows[i];
		int res = strcmp(name, current_address->name);
		if(res == 0) {
...
int main(int argc, char *argv[])
{
	...
	switch(action) {
		...
		case 's':
			...
			break;
		case 'f':
			if(argc != 4) die("Need name to find DB entry");
